...
--guard1 멤버한사람의  카트에 제품번호 존재여부(0:insert,1:update)
select count(*) product_count from cart where userid='guard1' and p_no=1;
--guard1 카트에 있는 1번 제품의 수량증가
update cart set cart_qty=cart_cart_qty+2 where userid='guard1' and p_no=1;


--카트아이템 1개의 제품정보(guard1 멤버한사람)

--guard1 카트에 있는 cart_no 1번의 수량3개 수정
update cart set cart_qty=3 where userid='guared1' and cart_no=1;
